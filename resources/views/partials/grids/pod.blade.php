...
<div class="content-box{{ $classes }} hover featured">
  ...
  <h4>Title</h4>
  Pod #@if(isset($position)){{$position}}@endif
</div>
@else
<div class="content-box{{ $classes }} <?php echo ( isset($pod['headline']) ) ? 'hover':''; ?> featured">
  <a href="{{ $pod['hyperlink'] }}" <?php if( $pod['hyperlink_target'] ){ echo 'target="_blank"'; } ?>>
    @if( count($pod['images']) > 0 )
      <img src="{{ config('filesystems.disks.rackspace.public_url') }}{{ $pod['images'][0]['url'] }}" />
    @else
  ...
  <h4>{{ $pod['title'] }}</h4>
  <a href="{{ URL::to('/pods/'.$pod->id.'/edit') }}" style="color: #000;">Edit Pod #@if(isset($position)){{$position}}@endif</a>
</div>
@endif
